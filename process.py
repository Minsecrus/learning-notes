...
""" + chunk_text
    ...
    for attempt in range(max_retries):
        try:
            print(f"Processing chunk {index+1}/{len(chunks)} (attempt {attempt+1})... length {len(chunk_text)}")
            ...
            with urllib.request.urlopen(req, timeout=300) as response:
                ...
                print(f"Chunk {index+1} finished in {time.time()-t0:.1f}s, new length {len(polished)}")
            ...
            print(f"Error on chunk {index+1}, attempt {attempt+1}: {err_msg}")
            time.sleep(10)

    print(f"Failed to process chunk {index+1}, returning original text.")
    return index, chunk_text

polished_chunks = [None] * len(chunks)

with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
    futures = [executor.submit(polish_chunk, i, chunk) for i, chunk in enumerate(chunks)]
    for future in concurrent.futures.as_completed(futures):
        idx, text = future.result()
        polished_chunks[idx] = text
...
print("Finished rewriting and saved to file.")
