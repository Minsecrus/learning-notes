...
调用 `socket(AF_INET, SOCK_STREAM)` 会创建一个 IPv4 TCP Socket。此时，它还只是一个毫无特征的“通用端点”，直到后续调用 `listen` 后，它才会被赋予“监听”的职责。我们在日志里打印的 `fileno()` 在 Windows 上返回的是一个标识该 *Socket 句柄*的整数，这个数字仅在当前进程的本次运行中有效。
...
`listen(BACKLOG)` 是一个关键的转折点，它将这个 Socket 正式转换为被动监听状态。传入的 *`BACKLOG`* 参数相当于告诉操作系统：“如果新的连接建好了，但我的应用程序还没来得及 `accept` 取走它，请在底层帮我暂存这些连接，最多存 `BACKLOG` 个”。不过，各个操作系统底层对这个数值可能会做微调或截断；而且不同平台在组织*半连接队列（SYN Queue）*和*全连接队列（Accept Queue）*的具体实现上也会有所差异。

::: details backlog 是什么？
backlog 是应用调用 `listen` 时给内核的监听排队容量提示，主要约束等待应用 `accept` 的已完成连接。内核可能按平台规则调整、截断或结合系统级上限解释这个值。

它并不直接等于“服务器最大并发连接数”；已被 `accept` 的连接、半连接队列和其他资源各有独立限制。
:::

::: details SYN Queue 是什么？
SYN Queue 常指服务端保存“已收到 SYN、握手仍未完成”状态的半连接队列。名称和实际数据结构随操作系统实现而异，SYN Cookie 等机制还可能减少为每个请求保留的状态。

队列压力过大时，新的握手可能被丢弃、延迟或转入保护机制。
:::

::: details Accept Queue 是什么？
Accept Queue 常指已经完成 TCP 握手、正在等待应用调用 `accept` 领取的连接队列。连接进入这里时，客户端通常已经认为握手成功，应用仍可能尚未取得对应 Socket。

应用处理过慢或队列容量不足会影响新连接建立，其具体表现取决于操作系统实现和配置。
:::
...
在这里，我们需要分清一个极易混淆的概念：*内核空间处理 TCP 握手*与*用户空间应用调用 `accept` 领取连接*，是两个独立的异步阶段。客户端发起的 TCP 握手可能早就在底层完成了，甚至连客户端发来的第一批业务数据都已经躺在操作系统的接收缓冲区（Receive Buffer）里了，这时候服务端的应用代码才慢吞吞地调用 `accept`。这种设计非常巧妙，操作系统底层的监听队列就像一个缓冲池，吸收了用户态应用程序在进程调度或处理业务时产生的短暂延迟。

::: details 内核空间是什么？
内核空间是操作系统核心代码和受保护数据运行的区域。TCP 状态机、路由、Socket 缓冲区和网卡驱动主要由内核维护，应用通过系统调用请求这些能力。

内核可以在应用线程尚未运行时继续处理到达的握手和确认报文。
:::

::: details 用户空间是什么？
用户空间是普通应用进程运行的受限区域。Python 对象、业务协议解析和应用队列都位于这里；应用需要通过 Socket API 在用户空间与内核网络栈之间传递数据和控制请求。

从内核缓冲区复制或映射到应用可见缓冲区后，业务代码才真正获得这些字节。
:::
