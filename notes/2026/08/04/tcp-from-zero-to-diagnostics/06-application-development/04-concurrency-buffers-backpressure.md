...
在这个公式中，$C$ 代表最大并发连接数，$Q$ 代表任务队列的容量上限，$S_{max}$ 是单个请求可能消耗的最大内存，$M_{out}$ 则是受*高水位线（High-Water Mark）*限制的待发送数据总量。

::: details 高水位线是什么？
高水位线是缓冲区或队列达到某个占用量后触发“暂停生产、暂停读取或限速”等动作的阈值。常与较低的恢复水位配对，积压下降到低水位后再恢复处理。

两个阈值形成滞回区间，可以减少系统在临界点附近频繁启停。
:::
...
::: details 线程池是什么？
线程池预先维护有限数量的工作线程，从任务队列中取出工作执行。它避免为每个请求反复创建线程，并用工作线程数限制同时执行的阻塞或计算任务数量。

线程数有限仍需配合有界任务队列；无限排队会把过载转化为内存增长和长尾延迟。
:::

虽然底层的并发模型改变了我们等待数据的方式，但请记住：协议的分帧解析、并发请求数限制、内存用量上限以及业务层面的*背压（Backpressure）*，仍然需要你在应用层代码中去显式地实现。

::: details 背压是什么？
背压是下游处理能力不足时，向上游传播“减慢、暂停或拒绝继续生产”的反馈。网络服务中，它可以从应用队列传到 Socket 接收缓冲区，再通过 TCP 接收窗口影响发送端。

反馈传播需要时间，因此还要限制请求大小、在途数量和各级队列容量，控制压力到达前已经进入系统的数据。
:::
...
合理的容量设计应该基于你“能接受的突发时长”来推算：假设服务的正常处理速率为 $\mu$，短时间内的请求到达峰值速率为 $\lambda$，持续了 $t$ 秒，那么这段时间里新增的积压排队量大约就是 $(\lambda-\mu)t$。任务应进入*有界队列（Bounded Queue）*；对于超出队列上限的部分，我们应该直接走*快速拒绝（Fast Reject）*逻辑，或者依赖上游系统来进行限流。

::: details 有界队列是什么？
有界队列具有明确最大容量，满载时提交者必须等待、降级或收到拒绝。它把最坏情况下的内存占用和排队数量限制在可计算范围内。

容量应依据可接受突发时长、处理速率和延迟目标推导，并通过队列占用率和等待时间持续监控。
:::

::: details 快速拒绝是什么？
快速拒绝是在系统容量耗尽时尽早返回明确的过载结果，例如 HTTP 503 或协议级 BUSY，并可附带退避提示。它让客户端及时换实例、降级或受控重试，也保护服务避免无限排队。

拒绝响应应在能够识别 Request ID 后尽量返回，以便调用方关联结果并执行幂等重试。
:::
