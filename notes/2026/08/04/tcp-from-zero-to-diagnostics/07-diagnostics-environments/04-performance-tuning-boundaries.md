...
要注意的是，TCP 参数只是整条处理链路中的一环。应用排队、数据库、CPU、内存、磁盘 IO、锁竞争、序列化、日志打印、网络带宽、RTT、丢包率、中间设备，甚至连接创建的频率，都可能决定最终的性能表现。
...
这里的每一项还能继续细化。比如服务端耗时 `T_service` 往往包含了应用队列等待、数据库查询、锁竞争以及下游服务调用；而请求耗时 `T_request` 和响应耗时 `T_response` 则会受到 RTT（往返时延）、TCP 窗口大小、拥塞控制（Congestion Control）、丢包率以及应用自身写入节奏的影响。
