...
在本章中，我们将*健康检查（Health Check）*拆解为三个可验证的问题：“网络路径是否还能响应 TCP？”“远端应用是否还在正常处理协议？”以及“底层业务依赖是否能够正常工作？”。

::: details 健康检查是什么？
健康检查是系统主动执行的探测，用来判断某个组件是否具备指定层次的服务能力。TCP 探测、应用心跳和真实依赖检查覆盖的路径不同，结论也应分别命名。

检查本身会消耗资源并可能命中缓存，设计时要明确频率、超时、成功条件和失败后动作。
:::
...
::: details TCP Keepalive 是什么？
TCP Keepalive 在连接长时间没有正常流量后，由内核发送小型探测并等待对端 TCP 栈回应。达到失败阈值后，内核向应用报告连接失效。

它验证的是连接状态与网络路径，通常由应用按 Socket 启用；远端业务线程卡死时，内核仍可能正常回复探测。
:::
...
*应用层心跳（Application Heartbeat）*通常由一对带有编号（ID）的 `PING/PONG` 消息组成，例如：

::: details 应用层心跳是什么？
应用层心跳是业务协议定义的周期性交互，例如带 Request ID 的 PING/PONG。它要求对端应用读取、解析并生成响应，因此能覆盖协议处理器、事件循环或线程调度等路径。

心跳成功仍只证明这条轻量处理链可用，真实业务依赖与权限逻辑需要更有代表性的检查。
:::
...
业务层的健康检查通常会真实调用一次关键的底层依赖，比如读取一条已知的数据记录，或是执行一笔没有副作用的“合成事务”。这类检查旨在回答：“当前服务是否具备完成目标业务操作的能力？”。像负载均衡器发起的*存活检查（Liveness Probe）*、容器编排系统 *Kubernetes* 的*就绪检查（Readiness Probe）*，以及端到端的合成监控，都属于健康检查体系，覆盖范围和控制动作各有不同。

::: details Liveness Probe 是什么？
Liveness Probe 判断进程或容器是否陷入需要重启才能恢复的故障。连续失败后，编排系统通常会重启容器。

它应聚焦本实例自身能否继续运行，避免把暂时的外部依赖故障直接转化为大规模重启风暴。
:::

::: details Kubernetes 是什么？
Kubernetes 是容器编排平台，负责部署、调度、扩缩容、服务发现和实例生命周期管理。它通过探针等机制决定容器何时重启、何时接收流量以及启动阶段何时完成。

探针的 HTTP/TCP/命令执行只是检测方式，返回语义需要应用与运维共同设计。
:::

::: details Readiness Probe 是什么？
Readiness Probe 判断实例当前是否适合接收新流量。失败时，Kubernetes 通常把实例从 Service 可用端点中移除，同时保留进程继续运行和恢复。

它可以覆盖必要依赖与过载状态，阈值应避免短暂抖动导致频繁摘除和加入。
:::
