...
- **捕获过滤器 (Capture Filter)**：在抓包记录前生效，用于从源头过滤掉不关心的数据（例如输入 `tcp port 8765`）。
- **显示过滤器 (Display Filter)**：在抓包完成后生效，用于在海量记录中筛选出特定的报文（例如输入 `tcp.port == 8765`）。
...
当控制台输出 `Serving HTTP on 127.0.0.1 port 8765` 时，请保持窗口开启。此时，该 Python 进程已经成功绑定在 IPv4 的回环地址 `127.0.0.1` 上，并正在监听 TCP 端口 `8765`。
...
- **端口已被占用**：此时可以换用 `8766` 等其他空闲的高位端口，或者利用 `Get-NetTCPConnection` 输出的 `OwningProcess`（进程 ID）找出占用该端口的幕后黑手。在强制结束该进程前，请先确认它并非关键系统服务。
